...
* <tt>interface</tt> - (optional) network interface from which to pull a 48-bit mac address as the worker id.  If not specified this defaults to eth0.
* <tt>worker_id</tt> - (optional) explicit worker id which, if present, overrides <tt>interface<tt>.
* <tt>timestamp_path</tt> - (optional) location where flake can periodically save the current time.  Defaults to "/tmp/flake-timestamp-dets".  If flake detects on startup that this file contains timestamps in the future or the distant past, it will refuse to startup. This is to prevent problematic ids from being distributed.  
